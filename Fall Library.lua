...
local enabled = false
			sliderButton.InputBegan:Connect(function(Input)
				if Input.UserInputType == Enum.UserInputType.MouseButton1 then
					...
					if enabled then
                        local color1tween = game:GetService("TweenService"):Create(framemain, TweenInfo.new(.2), {ImageColor3 = Color3.fromRGB(33, 148, 255)})
						local positiontween = game:GetService("TweenService"):Create(innerframe, TweenInfo.new(.12), {Position = UDim2.new(0.628, 0,0.112, 0)})
						positiontween:Play()
                        color1tween:Play()
					else
                        local color1tween = game:GetService("TweenService"):Create(framemain, TweenInfo.new(.2), {ImageColor3 = Color3.fromRGB(20, 20, 21)})
